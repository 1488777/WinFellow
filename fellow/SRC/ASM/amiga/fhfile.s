...
loop:		move.l	(a0,d0.l),(a5,d0.l)	; Copy packet to a5
		...
		moveq	#20,d0	; Alloc memory for bootnode
		bsr	AllocMem
...
relocateloop:
		...
		addq.l	#4,d0			; Make space for pointer to next segment
		bsr	AllocMem
		bsr RelocateHunk
...
AddFileSystemEntry:
		...
		move.l	d0,a4			; FileSysEntry to A4
;		lsr.l	#2,d7
		move.l	d7,54(a4)		; SegList in FileSysEntry
...
AddFileSystemEntries:
		...
		tst.l	d3
		beq		addfilesystementriesend
		moveq.l	#0,d1

addfilesystementriesloop:
		bsr	AddFileSystemEntry

		addq.l	#1,d1
		cmp.l	d1,d3
		bne	addfilesystementriesloop

addfilesystementriesend:
		rts
